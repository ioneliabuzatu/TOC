...
def main_control_steady_state():
    start = time.time()
    expert = CellStateClassifier(num_genes=100).cuda()
    env = Ducky(
    ...
    def loss_fn(actions):
        ...
        return -jnp.mean(jnp.sum(jnp.power(expr, 2), axis=1))

    actions = jnp.zeros((env.sampling_state_ * env.num_cells, env.num_cell_types, env.num_genes))
